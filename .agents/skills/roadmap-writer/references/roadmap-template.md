...
- [ ] **EX-3.1** Short step title
  ...
  - **Instructions:** Explicit, step-by-step technical directions detailing what needs to be coded, changed, or configured.
  - **Verify:** `go test ./...`
...
| **Instructions** | Yes         | Explicit, step-by-step technical directions detailing what to code, configure, or change   |
...
Repeat paths, repos, instructions, and verify commands in every step that needs them.
...
- [ ] **UR2-3.2** Add KOPDS storage user methods
  ...
  - **Instructions:** Implement the `GetUser` and `CreateUser` methods in `sqlite.go` to satisfy the `domain.UserRepository` interface. Ensure parameterized queries are used to prevent SQL injection.
...
- [ ] **UR2-2.3** Fix Docker publish workflows
  ...
  - **Instructions:** Update the `docker-publish.yml` workflow in both repositories to point to the correct Dockerfile path (`build/Dockerfile`) under the `docker/build-push-action` block.
...
2. Add a `Timeout` field to the struct.
3. Run `go test ./...`.
...
Do not leave "see previous step" or "as above" — repeat paths, instructions, and commands in every step.
...
- [ ] Every step lists **Repos**, **Instructions**, **Verify**, and **Done when**
