---
name: roadmap-writer
description: Authors implementation roadmaps as self-contained steps for stepwise-implementor. Use when creating, auditing, or rewriting a phased checklist roadmap.
---

# Roadmap Writer

Writes or rewrites implementation roadmaps as sequential, self-contained steps so **stepwise-implementor** can execute them with minimal context per step.

## When to Use This Skill

- When asked to generate a new implementation roadmap for a project.
- When asked to rewrite an existing roadmap to improve clarity, structure, or task granularity.
...
| Role | Skill |
|------|--------|
| **Author** | roadmap-writer — produces or updates `*.md` roadmaps |
| **Execute** | stepwise-implementor — one `[ ]` step at a time, commits, handoff |
...
## Context Management (Authoring)

- **Audit once** — a full codebase audit may be wide, but output is the roadmap markdown, not ongoing chat state.
...
- **Roadmap-only edits** — do not implement application code or commit in `kopds/` / `kosync/` unless the user explicitly asks. Root roadmap files commit in KOSERVER.
...
- Project `AGENTS.md` at the workspace root — multi-repo commit rules
...
- Convert legacy `1. Open ... 2. Run ...` lists into the template fields
