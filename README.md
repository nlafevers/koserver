...
KOSERVER is a deployment guide and sample-configuration set for two lightweight Go applications that together provide a complete self-hosted backend for [KOReader](https://koreader.rocks/) devices:
...
Both apps are built in pure Go, ship as single binaries, and are designed to run comfortably on the most resource-constrained self-hosting hardware — a Raspberry Pi, a free-tier cloud VM, or an old PC in a closet. This guide deploys them on a single machine, behind a single HTTPS reverse proxy ([Caddy](https://caddyserver.com/), which handles certificates for you automatically), so your devices can reach both securely from anywhere.
...
4.  [Part A — Native Install (recommended)](#-part-a--native-install-recommended)
5.  [Part B — Caddy Reverse Proxy & HTTPS](#-part-b--caddy-reverse-proxy--https)
6.  [Part C — Docker Alternative](#-part-c--docker-alternative)
7.  [Part D — Cloud: Free-Tier GCP VM](#-part-d--cloud-free-tier-gcp-vm)
8.  [Hardening & Operations](#-hardening--operations)
9.  [Connecting KOReader](#-connecting-koreader)
...
Handy commands for testing and operating a running deployment. Replace `<app>` with `kopds` or `kosync`, and substitute your own users, passwords, and paths.

### CLI

User management is built into each binary. The subcommands are identical for KOPDS and KOSYNC.
...
When an app runs in a container, use these from the host shell to check logs or run the CLI user-management commands.
...
Inspect the database directly to verify the effect of CLI commands or HTTP requests.
...
| Resource | Link |
| :--- | :--- |
| **KOPDS** (OPDS server) | https://github.com/nlafevers/kopds |
| **KOSYNC** (sync server) | https://github.com/nlafevers/kosync |
